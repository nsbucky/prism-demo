...
->withStringParameter('city', 'The city that might have spatulas')
->using(fn (string $city): string => "{$city} has the best!");

$response = Prism::text()
            ->using(Provider::Ollama, 'llama3.2')
            ->withMaxSteps(2)
            ->withPrompt('What city has the best spatulas?')
            ->withTools([$tool])
            ->withToolChoice('spatula')
            ->asText();`
